  ...
  return (
    <>
      <div className="mb-6 flex items-center justify-between">
        ...
        <div className="hidden gap-1 rounded-lg bg-dark-surface p-1 sm:flex">
          {["1D", "1W", "1M", "3M", "1Y", "ALL"].map((p) => (
            <span key={p} className={cn("rounded-sm px-3 py-1.5 text-xs font-medium transition", p === "ALL" ? "bg-brand-gold text-black" : "text-soft-gray")}>{p}</span>
          ))}
        </div>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
        {metrics.map((m) => (
          <div key={m.label} className="rounded-xl border border-dark-surface bg-dark-charcoal p-5 shadow-l1">
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-2">
        <div className="xl:col-span-2 rounded-xl border border-dark-surface bg-dark-charcoal p-5 shadow-l1">
          <h3 className="mb-4 text-sm font-medium text-white">Equity curve</h3>
          <EquityChart data={buildEquityCurve(trades, startingBalance)} />
        </div>

        <div className="rounded-xl border border-dark-surface bg-dark-charcoal p-5 shadow-l1">
