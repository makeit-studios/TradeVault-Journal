...
export function CsvImport() {
  ...
  return (
    <details className="relative" open={open} onToggle={(e) => setOpen((e.target as HTMLDetailsElement).open)}>
      <summary className="cursor-pointer text-xs text-soft-gray hover:text-brand-gold">
        CSV import
      </summary>
      <div className="absolute right-0 top-6 z-10 w-80 rounded-xl border border-dark-surface bg-dark-charcoal p-4 shadow-l2">
        ...
        {result ? (
          <div className="mt-3 space-y-1 text-xs">
            <p className={result.imported > 0 ? "text-success-green" : "text-error-red"}>
              {result.imported} / {result.total} trades imported
            </p>
            {result.errors.length > 0 ? (
              <div className="mt-1 max-h-32 overflow-y-auto space-y-0.5">
                {result.errors.map((err, i) => (
                  <p key={i} className="text-error-red">
